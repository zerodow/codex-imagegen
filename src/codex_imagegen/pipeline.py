...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        creds = generator.load_credentials()
        ...
        if args.baseline_image:
            ...
            generator.generate_to_file(
                args.baseline_prompt, baseline, refs=None, size=args.size, fmt=args.fmt,
                model=args.model, total_timeout=args.timeout, stall_timeout=args.stall_timeout,
                progress=progress, auth_data=creds,
            )
        ...
        for idx, scene in enumerate(scenes, start=1):
            ...
            try:
                generator.generate_to_file(
                    scene, out, refs=refs, size=args.size, fmt=args.fmt, model=args.model,
                    total_timeout=args.timeout, stall_timeout=args.stall_timeout,
                    progress=progress, auth_data=creds,
                )
                saved.append(out)
